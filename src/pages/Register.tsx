...
export default function RegisterPage() {
  ...
  const { registerWithPassword, loginWithGoogle } = useAuthStore();
  const navigate = useNavigate();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const { error } = await registerWithPassword(name, email, password);
    setLoading(false);
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Input id="password" type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1.5" placeholder="•••••••• (min 6 chars)" />
            </div>
